...
@endsection
@section('description')
    Mengelola Konten Benefit di Menu Home Website
@endsection
@section('content')
<div class="row">
    <div class="main-card mb-3 card col-12">
        <div class="card-body">
            <ul class="tabs-animated-shadow nav-justified tabs-animated nav">
                ...
                @if ( $i === 1)
                <li class="nav-item">
                    <a data-toggle="tab" href="#tab-{{$item->id}}" class="active nav-link"><span> Benefit {{$i}}</span></a>
                </li>
                @else
                <li class="nav-item">
                    <a data-toggle="tab" href="#tab-{{$item->id}}" class="nav-link"><span> Benefit {{$i}} </span></a>
                </li>
                @endif
                @php
                $i++;
            ...
            <div class="tab-content">
                ...
                <div class="tab-pane active" id="tab-{{$item->id}}" role="tabpanel">
                    <form action="{{url('/benefit/update', $item->id)}}" method="POST" enctype="multipart/form-data">
                        @csrf @method('patch')
